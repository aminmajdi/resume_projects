...
class RobotEventNotificationList {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.notifications.forEach((val) => {
      bufferOffset = RobotEventNotification.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new RobotEventNotificationList(null);
    ...
    for (let i = 0; i < len; ++i) {
      data.notifications[i] = RobotEventNotification.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.notifications.forEach((val) => {
      length += RobotEventNotification.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new RobotEventNotificationList(null);
    if (msg.notifications !== undefined) {
      ...
      for (let i = 0; i < resolved.notifications.length; ++i) {
        resolved.notifications[i] = RobotEventNotification.Resolve(msg.notifications[i]);
      }
    }
    else {
...
module.exports = RobotEventNotificationList;
