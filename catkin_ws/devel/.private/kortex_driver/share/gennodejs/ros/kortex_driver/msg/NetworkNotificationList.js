...
class NetworkNotificationList {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.notifications.forEach((val) => {
      bufferOffset = NetworkNotification.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new NetworkNotificationList(null);
    ...
    for (let i = 0; i < len; ++i) {
      data.notifications[i] = NetworkNotification.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.notifications.forEach((val) => {
      length += NetworkNotification.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new NetworkNotificationList(null);
    if (msg.notifications !== undefined) {
      ...
      for (let i = 0; i < resolved.notifications.length; ++i) {
        resolved.notifications[i] = NetworkNotification.Resolve(msg.notifications[i]);
      }
    }
    else {
...
module.exports = NetworkNotificationList;
