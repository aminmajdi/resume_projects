...
class pendulum:
    ...
    def move(self, angular_velocity, theta, action):
        def ODE(X, t):
            self.x = X[0]
            self.y = X[1]
            dxdt = (action-self.b*self.x-0.5*self.m*self.g*self.l*np.sin(self.y))/(self.m*np.power(self.l,2/3))
            dydt = self.x 
            return [dxdt, dydt]
        ...
        while theta_p>=2*np.pi:
            theta_p-=2*np.pi
        if angular_velocity_p>2*np.pi:
            angular_velocity_p=2*np.pi
        elif angular_velocity_p<-2*np.pi:
            angular_velocity_p=-2*np.pi
