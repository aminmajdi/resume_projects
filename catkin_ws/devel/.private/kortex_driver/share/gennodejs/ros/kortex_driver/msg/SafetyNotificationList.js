...
class SafetyNotificationList {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.notifications.forEach((val) => {
      bufferOffset = SafetyNotification.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new SafetyNotificationList(null);
    ...
    for (let i = 0; i < len; ++i) {
      data.notifications[i] = SafetyNotification.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.notifications.forEach((val) => {
      length += SafetyNotification.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new SafetyNotificationList(null);
    if (msg.notifications !== undefined) {
      ...
      for (let i = 0; i < resolved.notifications.length; ++i) {
        resolved.notifications[i] = SafetyNotification.Resolve(msg.notifications[i]);
      }
    }
    else {
...
module.exports = SafetyNotificationList;
